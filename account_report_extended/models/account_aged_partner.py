...
#from odoo.addons.account_asset.models import account

def _get_columns_name(self, options):
        columns = [{}]
        columns += [
            {'name': v, 'class': 'number', 'style': 'white-space:nowrap;'}
            for v in [_("JRNL"), _("Project"),_("Account"), _("Reference"), _("Not due on: %s") % format_date(self.env, options['date']['date']),
                      _("1 - 30"), _("31 - 60"), _("61 - 90"), _("91 - 120"), _("Older"), _("Total")]
        ]
        return columns
    
account_aged._get_columns_name = _get_columns_name
        ...
        if 'partner_%s' % (values['partner_id'],) in options.get('unfolded_lines'):
            for line in amls[values['partner_id']]:
                ...
                vals = {
                    ...
                    'columns': [{'name': v} for v in [aml.journal_id.code,aml.invoice_id.project_id_from_so.name,aml.account_id.code, self._format_aml_name(aml)]] +\
